...
8. **1k judo.** `satya.css` < 1000 lines — **met** (996 after extracting `.satya-draft-status*` into `satya-draft-status.css`). Draft state extracted from `satya-client.ts`. `SatyaPresence.tsx` — **NOT met this PR:** it grew to 1161 lines and is a tracked follow-up (decompose the draft plumbing into `satya-draft-*` before the next Satya change), not a claim of Done.
