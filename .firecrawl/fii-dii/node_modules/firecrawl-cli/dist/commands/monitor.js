...
exports.hasCreateFlagPayload = hasCreateFlagPayload;
exports.hasUpdateFlagPayload = hasUpdateFlagPayload;
...
async function monitorRequest(path, options, init = {}) {
...
function emit(payload, options) {
    ...
    (0, output_1.writeOutput)(text, options.output, !!options.output);
}
/**
 ...
 */
async function readJsonPayload(file, options = {}) {
    if (file === '-' ||
...
function hasCreateFlagPayload(opts) {
...
function hasUpdateFlagPayload(opts) {
    return Boolean(opts.name ||
        opts.goal ||
        opts.cron ||
        opts.schedule ||
 ...
 */
function monitorDashboardUrl(id, apiUrl) {
    const url = apiUrl || (0, config_1.getConfig)().apiUrl || DEFAULT_API_URL;
...
function buildCreateBody(opts) {
    ...
    const urls = opts.urls && opts.urls.length > 0
        ? opts.urls
        : opts.page
            ? [opts.page]
            : undefined;
    ...
    if (opts.scheduleText)
        schedule.text = opts.scheduleText;
    if (opts.timezone)
        schedule.timezone = opts.timezone;
    const targets = [];
    if (hasScrape)
        targets.push({ type: 'scrape', urls });
    if (hasCrawl)
        targets.push({ type: 'crawl', url: opts.crawlUrl });
    if (hasSearch) {
        const searchTarget = {
            type: 'search',
            queries: opts.queries,
        };
        if (opts.searchWindow)
            searchTarget.searchWindow = opts.searchWindow;
...
function createMonitorCommand() {
    const monitor = new commander_1.Command('monitor').description('Schedule recurring scrapes/crawls/searches and track content changes');
    // create
    commonOptions(monitor
        .command('create')
        .description('Create a monitor (flags, or JSON from file/stdin)')
        .argument('[file]', 'Path to JSON payload (use "-" or pipe stdin to read from stdin)')
        .option('--name <name>', 'Monitor name')
        .option('--cron <expression>', 'Cron schedule (e.g. "*/30 * * * *")')
        .option('--schedule <text>', 'Natural-language schedule (e.g. "every 30 minutes")')
        .option('--timezone <tz>', 'Schedule timezone', 'UTC')
        .option('--page <url>', 'Single page URL to scrape on each check')
        .option('--scrape-urls <list>', 'Comma-separated page URLs to scrape on each check', parseCommaList)
        .option('--crawl-url <url>', 'Root URL for a crawl target')
        .option('--queries <list>', 'Comma-separated search queries for a search target (requires --goal)', parseCommaList)
        .option('--search-window <window>', 'Search recency window: 5m, 15m, 1h, 6h, 24h, 7d (default: 24h)')
        .option('--max-results <n>', 'Max search results per query, 1-50 (default: 10)', parseInt)
        .option('--include-domains <list>', 'Comma-separated domains to restrict search results to', parseCommaList)
        .option('--exclude-domains <list>', 'Comma-separated domains to exclude from search results', parseCommaList)
        .option('--webhook-url <url>', 'Webhook destination')
        .option('--webhook-events <list>', 'Comma-separated events (monitor.page, monitor.check.completed)', parseCommaList)
        .option('--email <list>', 'Comma-separated email recipients for change notifications', parseCommaList)
        .option('--retention-days <n>', 'Snapshot retention window', parseInt)
        .option('--goal <text>', 'Plain-language goal for the AI change judge (auto-enables the judge)')
        .addHelpText('after', `
...
`)).action(async (file, options) => {
        try {
            ...
            if (createdId && process.stdout.isTTY) {
                ...
                if (link)
                    hints.push(`  Open in dashboard:  ${link}`);
    ...
    commonOptions(monitor
        ...
        .option('--offset <n>', 'Result offset', parseInt)).action(async (options) => {
        try {
            const payload = await monitorRequest('/monitor', options, {
    ...
    commonOptions(monitor
        ...
        .argument('<monitorId>', 'Monitor ID')).action(async (monitorId, options) => {
        try {
            const payload = await monitorRequest(`/monitor/${encodeURIComponent(monitorId)}`, options);
            emit(payload, options);
        }
        catch (err) {
            fail(err);
    ...
    commonOptions(monitor
        ...
        .option('--retention-days <n>', 'Snapshot retention window', parseInt)).action(async (monitorId, file, options) => {
        try {
            const fromJson = await readJsonPayload(file, {
            ...
            if (fromJson) {
                ...
                if (options.state)
                    body.status = options.state;
                if (options.retentionDays !== undefined)
                    body.retentionDays = options.retentionDays;
                if (options.cron || options.schedule || options.timezone) {
                    const schedule = {};
                    if (options.cron)
                        schedule.cron = options.cron;
                    if (options.schedule)
                        schedule.text = options.schedule;
                    if (options.timezone)
                        schedule.timezone = options.timezone;
                    body.schedule = schedule;
                }
                if (Object.keys(body).length === 0) {
                    throw new Error('Provide at least one field to update (or a JSON file / stdin payload)');
                }
            }
            const payload = await monitorRequest(`/monitor/${encodeURIComponent(monitorId)}`, options, { method: 'PATCH', body });
            emit(payload, options);
        }
        catch (err) {
            fail(err);
    ...
    commonOptions(monitor
        ...
        .argument('<monitorId>', 'Monitor ID')).action(async (monitorId, options) => {
        try {
            const payload = await monitorRequest(`/monitor/${encodeURIComponent(monitorId)}`, options, { method: 'DELETE' });
            emit(payload, options);
        }
        catch (err) {
            fail(err);
    ...
    commonOptions(monitor
        ...
        .argument('<monitorId>', 'Monitor ID')).action(async (monitorId, options) => {
        try {
            const payload = await monitorRequest(`/monitor/${encodeURIComponent(monitorId)}/run`, options, { method: 'POST' });
            emit(payload, options);
        }
        catch (err) {
            fail(err);
    ...
    commonOptions(monitor
        ...
        .argument('<monitorId>', 'Monitor ID')
        .option('--limit <n>', 'Maximum results', parseInt)
        .option('--offset <n>', 'Result offset', parseInt)).action(async (monitorId, options) => {
        try {
            const payload = await monitorRequest(`/monitor/${encodeURIComponent(monitorId)}/checks`, options, { query: { limit: options.limit, offset: options.offset } });
            emit(payload, options);
        }
        catch (err) {
            fail(err);
    ...
    commonOptions(monitor
        ...
        .option('--page-status <state>', 'Filter page results: same, new, changed, removed, error')).action(async (monitorId, checkId, options) => {
        try {
            const payload = await monitorRequest(`/monitor/${encodeURIComponent(monitorId)}/checks/${encodeURIComponent(checkId)}`, options, {
                query: {
                    limit: options.limit,
                    skip: options.skip,
