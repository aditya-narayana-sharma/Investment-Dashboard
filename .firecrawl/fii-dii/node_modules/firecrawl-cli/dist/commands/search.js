...
async function executeSearch(options) {
    try {
        ...
        if ((0, client_1.isKeylessMode)(options.apiKey, options.apiUrl)) {
            ...
            envelope = (await (0, client_1.keylessRequest)('/v2/search', searchBody));
        }
        else {
            const app = (0, client_1.getClient)({ apiKey: options.apiKey, apiUrl: options.apiUrl });
            const httpResponse = await app.http.post('/v2/search', searchBody);
            envelope = (httpResponse?.data ?? {});
        }
        const payload = (envelope.data ?? {});
        const data = {};
        if (payload.web)
            data.web = payload.web;
        if (payload.images)
