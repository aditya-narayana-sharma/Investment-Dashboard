...
exports.handleMultiScrapeCommand = handleMultiScrapeCommand;
exports.handleAllScrapeCommand = handleAllScrapeCommand;
...
async function executeScrape(options) {
    ...
    const resolvedFormats = formats.map((format) => {
        if (format === 'json' && options.schema) {
            return { type: 'json', schema: options.schema };
        }
        return format;
    });
    ...
    try {
        ...
        if (scrapeId) {
            ...
            try {
                (0, interact_session_1.saveInteractSession)({
                    scrapeId,
                    url: options.url,
                ...
                process.stderr.write(`Warning: Could not save scrape session. ` +
...
async function handleScrapeCommand(options) {
    ...
    if (options.query && result.success && result.data?.answer) {
        (0, output_1.writeOutput)(result.data.answer, options.output, !!options.output);
        return;
    }
...
async function handleMultiScrapeCommand(urls, options) {
    ...
    const promises = urls.map(async (url) => {
        const scrapeOptions = { ...options, url };
        const result = await executeScrape(scrapeOptions);
        const currentCount = ++completedCount;
        if (!result.success) {
            errorCount++;
            process.stderr.write(`[${currentCount}/${total}] Error: ${url} - ${result.error}\n`);
            return;
        }
 ...
 */
function urlToNestedPath(url, filename = 'index.md') {
    try {
        const parsed = new URL(url);
        const host = parsed.hostname.replace(/^www\./, '');
 ...
 */
async function runWizard(urls, options, allOptions) {
    ...
    if (topPaths.length > 1) {
        const pathChoices = await checkbox({
...
async function handleAllScrapeCommand(siteUrl, options, allOptions = {}) {
    let { limit, yes, search, includePaths, excludePaths, allowSubdomains } = allOptions;
    const fs = await Promise.resolve().then(() => __importStar(require('fs')));
    const path = await Promise.resolve().then(() => __importStar(require('path')));
    // Map from origin so non-root URLs (e.g. pasted subpage) work reliably
    const mapUrl = (0, url_1.getOrigin)(siteUrl);
    process.stderr.write(`Mapping ${mapUrl}...\n`);
    const mapResult = await (0, map_1.executeMap)({
        urlOrJobId: mapUrl,
        apiKey: options.apiKey,
        apiUrl: options.apiUrl,
        search,
    ...
    const status = await (0, status_1.getStatus)();
    ...
    if (status.credits) {
        ...
        if (creditsNeeded > remaining) {
            console.error(`Error: Not enough credits. Need ${creditsNeeded}, have ${remaining}.`);
            process.exit(1);
        }
    }
    if (!yes) {
    ...
    process.stderr.write(`Scraping ${urls.length}${limit ? ` of ${mapResult.data.links.length}` : ''} pages (${maxConcurrency} at a time)...\n`);
    ...
    const processUrl = async (url) => {
        ...
        if ((options.screenshot || options.fullPageScreenshot) &&
        ...
        process.stderr.write(`[${currentCount}/${total}] Saved: ${dir}/ (${savedFiles.length} files)\n`);
    };
    ...
    const workers = Array.from({ length: Math.min(maxConcurrency, urls.length) }, () => runWorker());
