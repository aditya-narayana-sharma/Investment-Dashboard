...
exports.executeCrawl = executeCrawl;
exports.handleCrawlCommand = handleCrawlCommand;
...
async function executeCrawl(options) {
    try {
        ...
        if (status || (0, job_1.isJobId)(urlOrJobId)) {
        ...
        if (wait) {
            ...
            if (options.progress) {
                ...
                while (true) {
                    ...
                    const status = await app.getCrawlStatus(jobId);
                    // Show progress
                    process.stderr.write(`\rProgress: ${status.completed}/${status.total} pages (${status.status})`);
...
function formatCrawlStatus(data) {
    ...
    if (data.expiresAt) {
        ...
        lines.push(`Expires: ${expiresDate.toLocaleString('en-US', {
            year: 'numeric',
            month: 'short',
            day: 'numeric',
            hour: '2-digit',
