...
export function KiteOrderTicket({
  selection,
  holdings,
  onClose,
  onSubmitted,
  ...
  const funds = buyOrderFundsState({
    side,
    quantity,
    estimatedPrice,
    equityMargin,
  ...
  const ready = kiteSessionLive && reviewed && confirmation.trim().toUpperCase() === expected && lookup.exact !== null && quantity >= 1 && (!needsPrice || Number(price) > 0) && (!needsTrigger || Number(triggerPrice) > 0);
  ...
  async function submitOrder() {
    ...
    try {
      ...
      const payload = await readKiteTicketResponse(response, "Kite order");
  ...
  return <KiteTicketPortal><div className="kite-order-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !submitting) onClose(); }}>
    <section className={`kite-order-ticket ${side.toLowerCase()}`} role="dialog" aria-modal="true" aria-labelledby="kite-order-title">
      ...
      {!kiteSessionLive && <div className="kite-order-warning kite-order-session" role="alert"><AlertTriangle size={18}/><span><b>Kite session required — log in on this Mac.</b> Orders cannot be submitted while Kite is unavailable, cached, or unauthenticated. The form stays open so you can retry after login.</span></div>}
      {funds.insufficient && <div className="kite-order-warning kite-order-funds" role="alert"><AlertTriangle size={18}/><span><b>{INSUFFICIENT_BUY_FUNDS_MESSAGE}</b></span></div>}
      ...
      <div className="kite-order-summary"><span>Estimated order value</span><b data-demo-sensitive="">{estimatedPrice > 0 ? inr.format(funds.requiredFunds || estimatedPrice * quantity) : "—"}</b><small>{matchedHolding ? "Kite holding last price." : lookup.publicPrice ? "Indicative public delayed close; Kite determines the actual fill." : "No quote entitlement is used for submission; market orders may fill at a different price."}{side === "BUY" && marginsKnown ? ` Available equity margin ${inr.format(equityMargin)}.` : ""}</small></div>
