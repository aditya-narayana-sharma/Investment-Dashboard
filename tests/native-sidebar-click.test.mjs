...
test("group titles match overview URLs and Stratji has no sidebar", async () => {
  ...
  assert.match(dashboard, /hoverRevealStripHeight/);
  assert.match(dashboard, /hideDelay/);
  assert.match(dashboard, /glassOverlay\.ignoresHits/);
  assert.match(dashboard, /func revealGlass/);
  ...
  assert.match(glassBar, /hoverRevealStripHeight: CGFloat = 14/);
  assert.match(glassBar, /hideDelay: TimeInterval = 0\.8/);
