#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
LOG_DIR="$HOME/Library/Logs/PortfolioIntelligence"
PID_FILE="$LOG_DIR/service.pid"

mkdir -p "$LOG_DIR"

if curl -sf --max-time 3 http://127.0.0.1:5050/_flask/health >/dev/null 2>&1; then
  printf 'Portfolio Intelligence is already running at http://localhost:5050/\n'
  exit 0
fi

if [[ -f "$PID_FILE" ]]; then
  OLD_PID="$(cat "$PID_FILE" 2>/dev/null || true)"
  if [[ -n "$OLD_PID" ]] && kill -0 "$OLD_PID" 2>/dev/null; then
    kill "$OLD_PID" 2>/dev/null || true
    for _ in {1..20}; do
      kill -0 "$OLD_PID" 2>/dev/null || break
      sleep .25
    done
  fi
  rm -f "$PID_FILE"
fi

# launchd cannot reliably traverse projects under ~/Documents without extra
# macOS privacy grants. Keep one supervised shell process instead; its cleanup
# trap owns both Vinext and Waitress.
nohup /bin/bash "$ROOT_DIR/scripts/run-dashboard-service.sh" \
  >>"$LOG_DIR/service.log" 2>&1 &
SERVICE_PID=$!
printf '%s\n' "$SERVICE_PID" >"$PID_FILE"

for _ in {1..75}; do
  if curl -sf --max-time 3 http://127.0.0.1:5050/_flask/health >/dev/null 2>&1; then
    printf 'Portfolio Intelligence is running at http://localhost:5050/\n'
    exit 0
  fi
  sleep 1
done

kill "$SERVICE_PID" 2>/dev/null || true
rm -f "$PID_FILE"
printf 'Portfolio Intelligence did not start. Check ~/Library/Logs/PortfolioIntelligence/.\n' >&2
exit 1
