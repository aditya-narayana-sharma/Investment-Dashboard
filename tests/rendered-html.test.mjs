...
test("server-renders the print report and keeps controls interactive", async () => {
  ...
  assert.match(contentServer, /analysisDayMessages\("Newsletters"/);
  ...
  assert.match(contentServer, /\[newsletters, axisResearch, podcasts, reminders, calendar, healthNote\] = await Promise\.all/);
  ...
  assert.match(packageJson, /"iphone": "\.\/scripts\/start-remote-app\.sh"/);
  ...
  assert.match(liveServer, /subSectorAllocation: allocations\(donutHoldings, "subSector", subSectorColors\)/);
  assert.match(page, /data=\{subSectorAllocation\}.*ring="subsector"/);
  assert.match(reportPage, /subSectors=\{snapshot\.subSectorAllocation\}/);
  assert.match(liveServer, /const donutHoldings = holdings\.slice\(\)\.sort\(\(a, b\) =>[\s\S]*a\.marketCap\}[|]\$\{a\.sector\}[|]\$\{a\.subSector\}/);
  ...
  assert.match(sectorServer, /callKiteTool\("get_ohlc"/);
  assert.match(sectorServer, /callKiteTool\("get_ltp"/);
  assert.match(sectorServer, /quoteMode = "OHLC fallback"/);
