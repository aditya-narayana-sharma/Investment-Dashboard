...
export const kanbanItems: Record<KanbanWorkspace, KanbanItem[]> = {
  investment: [
    { id: "inv-kite", title: "Refresh Kite and validate holdings", detail: "Reconcile holdings, positions, orders and GTTs before acting.", numericAdvantage: "100% live coverage", strategicAdvantage: "Prevents stale decisions", sourceLabel: "Kite", sourceKind: "source", lane: "today", tone: "blue" },
    { id: "inv-concentration", title: "Review top-two concentration", detail: "Dilute the largest weights before adding more capital.", numericAdvantage: "Target <65% top-two", strategicAdvantage: "Improves shock resilience", sourceLabel: "Kite", sourceKind: "source", lane: "today", tone: "amber" },
    { id: "inv-macro", title: "Monitor oil, INR and institutional flows", detail: "Apply the macro triggers before increasing high-beta exposure.", numericAdvantage: "5 regime signals", strategicAdvantage: "Links macro to action", sourceLabel: "Macro", sourceKind: "source", lane: "monitor", tone: "red" },
    { id: "inv-earnings", title: "Update post-result theses", detail: "Fill KPI fields only after official results are published.", numericAdvantage: "4 KPIs per event", strategicAdvantage: "Reduces narrative bias", sourceLabel: "Earnings", sourceKind: "source", lane: "monitor", tone: "green" },
  ],
  sectors: [
    { id: "sec-breadth", title: "Refresh sector breadth and rankings", detail: "Validate prices, horizons and constituent coverage.", numericAdvantage: "11 sector universes", strategicAdvantage: "Separates leadership from single-stock moves", sourceLabel: "Sectors", sourceKind: "source", lane: "today", tone: "blue" },
    { id: "sec-kpis", title: "Check sector KPI freshness", detail: "Review each metric’s source date before using it.", numericAdvantage: "30 numeric KPI cards", strategicAdvantage: "Makes stale evidence visible", sourceLabel: "Sectors", sourceKind: "source", lane: "today", tone: "green" },
    { id: "sec-framework", title: "Run the selected sector through frameworks", detail: "Use PESTEL, Porter, life-cycle and structure together.", numericAdvantage: "4 independent lenses", strategicAdvantage: "Reduces one-factor conclusions", sourceLabel: "Frameworks", sourceKind: "source", lane: "monitor", tone: "amber" },
    { id: "sec-earnings", title: "Fill pending earnings KPIs", detail: "Keep unpublished values blank. Official releases only.", numericAdvantage: "0 fabricated values", strategicAdvantage: "Preserves research integrity", sourceLabel: "Earnings", sourceKind: "source", lane: "monitor", tone: "red" },
  ],
  /** Built per last NSE session from digest + verified earnings (`intelligence-daily-actions.ts`). */
  intelligence: [],
  health: [
    { id: "health-sync", title: "Verify the operational Health target", detail: "After 8 PM, confirm the newest archive advances the target date.", numericAdvantage: "8 PM date roll", strategicAdvantage: "Keeps the record auditable", sourceLabel: "Health", sourceKind: "source", lane: "today", tone: "blue" },
    { id: "health-averages", title: "Reconcile weekly and monthly averages", detail: "Show trends only where a complete comparison window exists.", numericAdvantage: "7-day + 30-day", strategicAdvantage: "Avoids overreading one day", sourceLabel: "Health", sourceKind: "source", lane: "today", tone: "green" },
    { id: "health-sleep", title: "Resolve cross-app sleep variance", detail: "Keep Apple Health primary. Treat Guava as a separate comparison.", numericAdvantage: "2-source check", strategicAdvantage: "Prevents merged incompatible totals", sourceLabel: "Health", sourceKind: "source", lane: "monitor", tone: "amber" },
    { id: "health-diary", title: "Complete nutrition diary", detail: "Treat logged intake as incomplete until every meal is entered.", numericAdvantage: "100% meal coverage", strategicAdvantage: "Improves nutrition signal", sourceLabel: "Health", sourceKind: "source", lane: "monitor", tone: "red" },
  ],
  builder: [
    { id: "builder-validate", title: "Validate the strategy tree", detail: "Confirm Weight percents, If/Else operands and the compiled graph.", numericAdvantage: "0 invalid trees", strategicAdvantage: "Blocks broken logic leaving the canvas", sourceLabel: "Canvas", sourceKind: "source", lane: "today", tone: "blue" },
    { id: "builder-asset", title: "Add Indian assets on the tree", detail: "Resolve symbols against live Kite holdings and watchlist.", numericAdvantage: "Holdings ∪ watchlist", strategicAdvantage: "Keeps sleeves on real NSE names", sourceLabel: "Canvas", sourceKind: "source", lane: "today", tone: "green" },
    { id: "builder-export", title: "Export the tree plus compiled graph", detail: "Save StrategyTreeV1 with compiled schemaVersion 2 before switching machines.", numericAdvantage: "Round-trip identical tree", strategicAdvantage: "Protects canvas work", sourceLabel: "Canvas", sourceKind: "source", lane: "monitor", tone: "amber" },
    { id: "builder-else", title: "Fill or accept an empty ELSE", detail: "THEN can hold assets. Empty ELSE is allowed and warned.", numericAdvantage: "THEN / ELSE wells", strategicAdvantage: "Completes the condition", sourceLabel: "Canvas", sourceKind: "source", lane: "monitor", tone: "red" },
  ],
  strategies: [
    { id: "strat-review", title: "Review the nine NSE library trees", detail: "Open a card to read the vertical tree before sending it to Canvas.", numericAdvantage: "9 NSE ETF trees", strategicAdvantage: "Keeps research on Indian sleeves", sourceLabel: "Library", sourceKind: "source", lane: "today", tone: "blue" },
    { id: "strat-stats", title: "Do not treat empty OOS tiles as live alpha", detail: "Library KPIs stay em dash until an Indian-market engine run exists.", numericAdvantage: "— until ran", strategicAdvantage: "Prevents fabricated performance", sourceLabel: "Library", sourceKind: "source", lane: "today", tone: "green" },
    { id: "strat-market", title: "Confirm every leaf is a Nifty 500 name", detail: "Use official NSE names only. No SPY, TQQQ, or SOXL.", numericAdvantage: "NSE only", strategicAdvantage: "Keeps Stratji on Indian markets", sourceLabel: "Library", sourceKind: "source", lane: "monitor", tone: "amber" },
    { id: "strat-open", title: "Open one tree in Algorithm Canvas", detail: "Deep-link a reconstruction into the editor. The library stays read-only.", numericAdvantage: "Canvas deep-link", strategicAdvantage: "Edits stay on the canvas", sourceLabel: "Library", sourceKind: "source", lane: "monitor", tone: "red" },
  ],
};
